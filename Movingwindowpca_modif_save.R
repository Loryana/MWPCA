...
window_pca <- function(H,genotype=gen,datagen_table=data.frame(datagen),datagen_test=data.frame(datagen),nco=10,date_gen=date_gen,date_gen_test=date_gen,datemax=max(date_gen),rnirmeth=TRUE){
  comp=1
  tcarre_val=c()
  sspe_res=c()
  ...
  for(i in 1:(datemax-H-1)){
    ...
    save(res_rnirs,file=paste(gen,'_w',H+1,'_iter',i,'_ncomp',nco,'.RData',sep=''))
